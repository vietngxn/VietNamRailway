...
public class GiaoDienHoanTraVe extends Application {
	...
	public VBox taoDataChoTableHoanVe(String mave, String chuyen, String gaDiGaDen, String trangThai, String vitrighe,
			String ngayMua, String hoten, String doituong, String sogiayto, double giave, String giamdoituong,
			String khuyenmai, double thanhtien, double phiHoanTra) {
...
//		});
		...
		VBox pnlsubCT6 = taoSubCT2("Phí hoàn trả", nf.format(phiHoanTra), lblCTStyle, lblValueCTStyle);
					...
					try {
						...
						pnlDataHoanTraVe.getChildren()
								.add(taoDataChoTableHoanVe(x.getMaVeTau(), x.getChuyenTau().getMaChuyenTau(),
										...
										"Toa số " + x.getSoToa() + " chỗ " + x.getSoGhe(), x.getTrangThaiVe(),
										...
										x.tinhThanhTien(), x.tinhPhiChenhLech(0, x.tinhThanhTien(), 10.0)));
	...
	public VBox loadDuLieuLenTable() throws SQLException {
		...
		for (Ve x : list) {
			box.getChildren().add(taoDataChoTableHoanVe(x.getMaVeTau(), x.getChuyenTau().getMaChuyenTau(),
					x.getGaDi() + " - " + x.getGaDen(),
					x.getNgayGioDi().toLocalDate() + " - " + x.getNgayGioDi().toLocalTime(),
					"Toa số " + x.getSoToa() + " chỗ " + x.getSoGhe(), x.getTrangThaiVe(), x.getKhachHang().getHoten(),
					x.getDoiTuongGiamGia().getTenDoiTuongGiamGia(), x.getKhachHang().getCccd(), x.getGiaVe(),
					nf.format(x.getDoiTuongGiamGia().getGiaTriPhanTramGiamGia()) + "%",
					nf.format(x.getKhuyenMai().getGiaTriPhanTramKhuyenMai()) + "%", x.tinhThanhTien(),
					x.tinhPhiChenhLech(0, x.tinhThanhTien(), 10.0)));
		}
		return box;
	}
