...
import fourcore.dao.*;
import fourcore.util.KiemTraThoiGian;
...
public class GiaoDienDoiVe extends Application {
    ...
    public VBox taoDataChoTableCapLaiVe(String maVeTau, String tenTau, String gaDigaDen, LocalDateTime ngayGioDi,
                                        ...
                                        String ghiChu, String trangThaiDoiVe, String trangThaiVe, ChuyenTau chuyenTau, KhachHang khachHang,
                                        KhuyenMai khuyenMai, DoiTuongGiamGia doiTuongGiamGia) throws SQLException {
        ...
        Label[] labels = { lbl_maVeTau = new Label(maVeTau), lbl_tenTau = new Label(tenTau),
                ...
                lbl_vitrighe = new Label("Toa số "+Integer.valueOf(soToa)+" chỗ " +Integer.valueOf(soGhe).toString()), lbl_trangthai = new Label(trangThaiVe) };
        ...
        data.setOnMouseEntered(e -> {
            // Chỉ hover nếu KHÔNG phải panel đang được chọn
            if (selectedPanel != pnlReturn) {
        ...
        data.setOnMouseExited(e -> {
            // Chỉ reset khi KHÔNG phải panel đang được chọn
            if (selectedPanel != pnlReturn) {
                data.setStyle(normalStyle);
...
//        VBox pnlsubCT2 = taoSubCT2("Giá vé", nf.format(ve1.getGiaVe()), lblCTStyle, lblValueCTStyle);
        ...
        VBox pnlsubCT5 = taoSubCT2("Thành tiền", String.format("%, .0f đ", giave), lblCTStyle, lblValueCTStyle);
        ...
        pnlReturn.getProperties().put("detail", pnlThongTinChiTiet);
        pnlReturn.getProperties().put("data", data);

        pnlReturn.setOnMouseClicked(event -> {
            try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("veDoi.dat"))) {
                oos.writeObject(null);
            } catch (IOException ex) {
                ex.printStackTrace();
            }
            // ===== 1. Nếu đang mở 1 panel khác → đóng nó =====
            if (selectedPanel != null && selectedPanel != pnlReturn) {

                HBox oldDetail = (HBox) selectedPanel.getProperties().get("detail");
                GridPane oldData = (GridPane) selectedPanel.getProperties().get("data");

                if (oldDetail != null) {
                    oldDetail.setVisible(false);
                    oldDetail.setManaged(false);
                }
            ...
            // ===== 2. Toggle panel hiện tại =====
            boolean isOpening = !pnlThongTinChiTiet.isVisible();

            pnlThongTinChiTiet.setVisible(isOpening);
            pnlThongTinChiTiet.setManaged(isOpening);
            data.setStyle(isOpening ? hoverStyle : normalStyle);

            // ===== 3. Xử lý dữ liệu vé =====
            if (isOpening) {
                selectedPanel = pnlReturn;
                String[] parts = gaDigaDen.split("\\s*-\\s*");

                String gaDi = parts[0];
                String gaDen = parts[1];
                vechon = new Ve(
                        maVeTau,
                        gaDi,
                        gaDen,
                ...
                maveTauchon = maVeTau;
                try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("veDoi.dat"))) {
                    oos.writeObject(vechon);
                } catch (IOException ex) {
                    ex.printStackTrace();
                }
                System.out.println("Dang chon ve: "+ vechon.getMaVeTau());
                ...
                System.out.println("Bo chon ve: "+ vechon.getMaVeTau());
                selectedPanel = null;
                vechon = null;
                maveTauchon = null;
            }
    ...
    public void start(Stage primaryStage) {
        try {
            ...
            VBox.setMargin(pnlCapNhatVe, new Insets(30, 0, 0, 900));
            ...
            pnlCapNhatVe.getChildren().addAll(btnDoiVe);

            btnDoiVe.setOnAction(e -> {
                KiemTraThoiGian kt = new KiemTraThoiGian();

                System.out.println("click doi ve");
                if(vechon==null){
                    Alert alert2 = new Alert(Alert.AlertType.ERROR);
                    alert2.setTitle("Lỗi");
                    alert2.setHeaderText(null);
                    alert2.setContentText("Vui lòng chọn vé cần đổi");
                    Stage stage2 = (Stage) ((Node) e.getSource()).getScene().getWindow();
                    alert2.initOwner(stage2);
                    alert2.initModality(Modality.WINDOW_MODAL);
                    alert2.showAndWait();
                }else if(!kt.isMoreThan24Hours(vechon.getChuyenTau().getNgayGioDi()) || vechon.getTrangThaiVe().equals("đã hoàn trả") || vechon.getTrangThaiVe().equals("Kết thúc")){
                    Alert alert2 = new Alert(Alert.AlertType.ERROR);
                    alert2.setTitle("Lỗi");
                    alert2.setHeaderText(null);
                    alert2.setContentText("Vé không đủ điều kiện đổi");
                    Stage stage2 = (Stage) ((Node) e.getSource()).getScene().getWindow();
                    alert2.initOwner(stage2);
                    alert2.initModality(Modality.WINDOW_MODAL);
                    alert2.showAndWait();
                    ...
                    try {
                        if(vedao.checkVeDoi(vechon.getMaVeTau())){
                            Alert alert2 = new Alert(Alert.AlertType.ERROR);
                            alert2.setTitle("Lỗi");
                            alert2.setHeaderText(null);
                            alert2.setContentText("Mỗi vé chỉ được đổi một lần");
                            Stage stage2 = (Stage) ((Node) e.getSource()).getScene().getWindow();
                            alert2.initOwner(stage2);
                            alert2.initModality(Modality.WINDOW_MODAL);
                            alert2.showAndWait();
                            ...
                            popupStage.setTitle("Chọn ngày muốn đổi");
                            ...
                            btnApDung.setAlignment(Pos.CENTER);
                            btnApDung.setTranslateX(120);
        //                    btnApDung.setOnMouseClicked(e3 ->{
        //
        //                    });
                            root1.getChildren().addAll(date,description, btnApDung);
            ...
            btnTimKiem.setOnMouseClicked(event -> {
                ...
                if (input.equals("")) {
                    ...
                    alert2.setTitle("Lỗi");
                    alert2.setHeaderText(null);
                    alert2.setContentText("Mã vé không hợp lệ! (Định dạng hợp lệ: VX123)");
                    ...
                    try {
                        ...
                        pnlDataDoiVe.getChildren()
                                .add(taoDataChoTableCapLaiVe(x.getMaVeTau(), x.getChuyenTau().getMaChuyenTau(),
                                        x.getGaDi() + " - " + x.getGaDen(), x.getNgayGioDi(), // ngayGioDi
                                        // (LocalDateTime)
                                        ...
                                        x.getChuyenTau(), // maChuyenTau
                                        x.getKhachHang(), // maKhachHang
                                        x.getKhuyenMai(), // maKhuyenMai
                                        x.getDoiTuongGiamGia()));
...
//            primaryStage.show();
    ...
    public void hienThi() throws SQLException {
        ...
        for (Ve x : list) {
            VBox dataPanel  =taoDataChoTableCapLaiVe(x.getMaVeTau(), x.getChuyenTau().getMaChuyenTau(),
                    x.getGaDi() + " - " + x.getGaDen(), x.getNgayGioDi(), // ngayGioDi (LocalDateTime)
                    x.getNgayGioDen(), // ngayGioDen (LocalDateTime)
                    x.getSoToa(), // soToa
                    x.getSoTang(), // soTang
                    x.getSoGhe(), // soGhe
