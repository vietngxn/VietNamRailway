...
public class HoaDonHoanTraExportPDF {

	private ChuyenTauDAO ctDAO;
	NumberFormat nf = NumberFormat.getInstance(new Locale("vi", "VN"));

	public void exportHoaDon(HoaDon hoaDon, ThongTinCtHoaDonHoanTraVe chiTietList) throws IOException, SQLException {
		...
		// Tiêu đề
		Paragraph title = new Paragraph("Hóa đơn hoàn trả vé").setFontSize(20).setBold()
		...
		document.add(
				new Paragraph("Đơn vị bán: Tổng công ty đường sắt Việt Nam                     Nhân viên bán hàng: Nguyễn Hữu Tiến"));
		...
		// Header bảng
		String[] headers = { "STT", "Mã vé", "Ga đi - ga đến", "Giá vé", "Phí hoàn trả", "Thành tiền" };

		for (String h : headers) {
			table.addHeaderCell(new Cell().add(new Paragraph(h).setBold().setFontSize(8)) // nhỏ font để gọn hơn
		...
		list = chiTietList.getList();
		double tongCong = 0;
		for (Entry<Ve, Double> entry : list.entrySet()) {
			Ve v = entry.getKey();
			double val = entry.getValue();
			table.addCell(String.valueOf(stt++));
			table.addCell(v.getMaVeTau());
			table.addCell(v.getGaDi() + " - " + v.getGaDen());
			table.addCell(nf.format(v.getGiaVe()) + "đ");
			table.addCell(nf.format(v.getGiaVe() - val) + "đ");
			table.addCell(nf.format(val) + "đ");
			tongCong += val;
		}
...
//        document.add(pThanhToan);
		...
		// document.add(new Paragraph("Số tiền bằng chữ: " +
		// tienBangChu).setTextAlignment(TextAlignment.RIGHT).setItalic());
